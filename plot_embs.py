...
acc_embed=np.load(os.path.join('output','arrays',run_name,'acc_mu.npy'))
spk_embed=np.load(os.path.join('output','arrays',run_name,'spk_mu.npy'))
embedding_acc_id=np.load(os.path.join('output','arrays',run_name,'acc_id.npy'))
embedding_spk_id=np.load(os.path.join('output','arrays',run_name,'spk_id.npy'))
...
# indexlist=[0,1,2,3,4,5,6,7,10,12,15,18] # L2ARCTIC indeces
indexlist=[0,1,2,3,4,5,6,7,10,12,15,18] # L2CMU indeces for 1 female+1 male (I am picking m1 and m3)
[0,1,2,3,4,5,6,7,11,14,17,18,21,22]
noindexlist=[]
nospklist=[]
