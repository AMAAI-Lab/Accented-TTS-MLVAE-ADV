...
def log(
    logger, step=None, losses=None, lossesD=None, grad_norm=None, fig=None, audio=None, sampling_rate=22050, tag=""
):
    if losses is not None:
        ...
        logger.add_scalar("Loss/acc_kl_loss", losses[4], step)
        logger.add_scalar("Loss/spk_kl_loss", losses[5], step)
        logger.add_scalar("Loss/acc_adv_loss", losses[6], step)
        logger.add_scalar("Loss/acc_ce_loss", lossesD[0], step)
...
def infer_one_sample(targets, predictions, vocoder, mel_stats, model_config, preprocess_config, path, args):
    ...
    if args.mode == "single":
        ...
        if args.accent and args.accent2 and args.accw>0:
            basename = args.accent + str(args.accw) + "_" + args.accent2 + str(args.accw2) + acf + "_siga" + str(args.siga) + spf + "_sigs" + str(args.sigs) + "_" + basename
        elif args.accent:
            basename = args.accent + acf + "_siga" + str(args.siga) + spf + "_sigs" + str(args.sigs) + "_" + basename
        else:
            basename = args.basename + "_" + basename
    elif args.mode == "batch":
        basename = "batch_" + str(targets[2][0].item()) + "_" + basename
    elif args.mode == "sample_stats":
...
def infer_one_sample_no_figure(targets, predictions, vocoder, mel_stats, model_config, preprocess_config, path, args):
