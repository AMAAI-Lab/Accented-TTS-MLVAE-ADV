...
#            return None
    ...
    def build_from_path(self):
        root_dir = os.path.join(self.in_dir,self.wav_tag)
#        txt_dir = os.path.join(self.in_dir,self.wav_tag,self.txt_dir)
#        wav_dir = os.path.join(self.in_dir,self.wav_tag, self.wav_dir)
        if self.speaker_emb != None:
...
#            skip_speakers = set()
        ...
        for spker_id, speaker in enumerate(tqdm(sub_dirs)):
            ...
            txt_dir = os.path.join(self.in_dir,self.wav_tag,speaker,self.txt_dir)
            wav_dir = os.path.join(self.in_dir,self.wav_tag,speaker,self.wav_dir)
            for i, txt_name in enumerate(tqdm(os.listdir(os.path.join(txt_dir)))):

                basename = txt_name.split(".")[0]
                with open(os.path.join(txt_dir, txt_name), "r") as f:
                    text = f.readline().strip("\n")

                wav_path = os.path.join(wav_dir, "{}.wav".format(basename, self.wav_tag))
                if not os.path.isfile(wav_path):
                    print("[Error] No wav file:{}".format(wav_path))
                    continue
